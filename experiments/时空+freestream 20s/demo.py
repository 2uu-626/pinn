...
def startup_ramp_factor_at_time(
    t: np.ndarray | float,
    tmin: float,
    tmax: float,
    mode: str = "ramp",
    ...
    """Return the shared startup factor used by inflow and rotor speed."""
    tt = np.asarray(t, dtype=float)
    mode_l = str(mode).lower()
    if mode_l in {"steady", "constant", "uniform"}:
        return np.full_like(tt, 1.0, dtype=float)
...
def startup_ramp_integral(
    ...
    """Return integral of the shared startup factor from tmin to t."""
    tt = np.asarray(t, dtype=float)
    mode_l = str(mode).lower()
    if mode_l in {"steady", "constant", "uniform"}:
        return tt - float(tmin)
    if mode_l not in {"ramp", "smooth_ramp", "startup"}:
        raise ValueError("INFLOW_MODE must be 'ramp' or 'steady'.")

    total_dt = max(float(tmax) - float(tmin), 1e-12)
    ramp_dt = max(float(ramp_fraction), 1e-6) * total_dt
    elapsed = tt - float(tmin)
    tau = np.clip(elapsed / ramp_dt, 0.0, 1.0)
    ramp_integral = ramp_dt * (tau**3 - 0.5 * tau**4)
    post_ramp = np.maximum(elapsed - ramp_dt, 0.0)
    return np.where(elapsed <= 0.0, 0.0, ramp_integral + post_ramp)
...
def inflow_speed_at_time(
    ...
    """
    return float(u_max) * startup_ramp_factor_at_time(
        t,
        tmin,
        tmax,
        mode=mode,
...
def rotor_omega_at_time(
    ...
    """Return rotor angular speed synchronized with the inflow startup."""
    return float(omega_rad_s) * startup_ramp_factor_at_time(
        t,
        tmin,
        tmax,
        mode=mode,
...
def rotor_phase_at_time(
    ...
    """Return rotor phase angle from integrating the synchronized omega(t)."""
    return float(omega_rad_s) * startup_ramp_integral(
        t,
        tmin,
        tmax,
        mode=mode,
...
def build_rotating_blade_dataset(
    ...
    omega_rad_s: float,
    t_ref: float = 0.0,
    motion_mode: str = "steady",
    ramp_fraction: float = 1.0,
    ramp_tmax: float | None = None,
) -> np.ndarray:
    surface_xyz = np.asarray(surface_xyz, dtype=float)
    t_values = np.linspace(float(tmin), float(tmax), int(num_t))
    phase_tmax = float(tmax if ramp_tmax is None else ramp_tmax)
    blade_blocks = []
    for ti in t_values:
        angle = float(
            rotor_phase_at_time(
        ...
        vel_t = rotating_wall_velocity(xyz_t, center_xyz, axis_xyz, omega_t)
...
def remove_points_inside_rotating_mesh(
    points_xyzt: np.ndarray,
    mesh,
    center_xyz: np.ndarray,
    axis_xyz: np.ndarray,
    ...
    if pts.shape[0] == 0:
        return pts
    phase_tmax = float(np.max(pts[:, 3]) if ramp_tmax is None else ramp_tmax)
    angles = -rotor_phase_at_time(
        pts[:, 3],
        float(t_ref),
        phase_tmax,
        omega_rad_s,
...
def remove_points_inside_obstacles(
    ...
    pts = remove_points_inside_rotating_mesh(
        points_xyzt,
        rotating_mesh,
        center_xyz,
        axis_xyz,
...
def build_freestream_anchor_dataset(
    ...
    ramp_fraction: float,
    static_mesh=None,
    radial_min_r: float = 2.2,
    oversample_factor: int = 5,
    rotor_motion_kwargs: dict | None = None,
) -> np.ndarray:
    """Build supervised free-stream points away from the rotor axis and wake core."""
    n_samples = int(n_samples)
    if n_samples <= 0:
        return np.empty((0, 7), dtype=float)

    center = np.asarray(rotor_center, dtype=float).reshape(1, 3)
    axis = _normalize(np.asarray(rotor_axis, dtype=float)).reshape(1, 3)
    rotor_radius = max(float(rotor_radius), 1e-8)
    radial_threshold = float(radial_min_r) * rotor_radius
    oversample_factor = max(1, int(oversample_factor))

    lb4 = np.array([domain["xmin"], domain["ymin"], domain["zmin"], tmin], dtype=float)
    ub4 = np.array([domain["xmax"], domain["ymax"], domain["zmax"], tmax], dtype=float)

    kept = []
    attempts = 0
    motion_kwargs = dict(rotor_motion_kwargs or {})
    while sum(block.shape[0] for block in kept) < n_samples and attempts < 20:
        attempts += 1
        cand = sample_box_lhs(lb4, ub4, n_samples * oversample_factor)
        xyz = cand[:, :3]
        rel = xyz - center
        axial_vec = np.sum(rel * axis, axis=1, keepdims=True) * axis
        radial = np.linalg.norm(rel - axial_vec, axis=1)
        cand = cand[radial >= radial_threshold]
        if cand.shape[0] == 0:
            continue

        cand = remove_points_inside_obstacles(
            cand,
            rotating_mesh,
            center.reshape(3),
            axis.reshape(3),
    ...
    if not kept:
        raise RuntimeError("Failed to generate freestream anchor points.")

    free_xyzt = np.vstack(kept)[:n_samples]
    if free_xyzt.shape[0] < n_samples:
        raise RuntimeError(
            f"Generated only {free_xyzt.shape[0]} freestream anchor points; requested {n_samples}."
        )

    return attach_inflow_profile(
        free_xyzt,
        tmin,
        tmax,
        u_max,
...
class PINNBladeRotatingUnsteady3D:
    def __init__(
        ...
        self.loss_weight_free = float(loss_weight_free)
        self.has_free_anchor = FREE is not None and np.asarray(FREE).shape[0] > 0
        ...
        if self.has_free_anchor:
            FREE = np.asarray(FREE, dtype=float)
            self.x_FREE = torch.tensor(FREE[:, 0:1], dtype=torch.float32, device=self.device)
            self.y_FREE = torch.tensor(FREE[:, 1:2], dtype=torch.float32, device=self.device)
            self.z_FREE = torch.tensor(FREE[:, 2:3], dtype=torch.float32, device=self.device)
            self.t_FREE = torch.tensor(FREE[:, 3:4], dtype=torch.float32, device=self.device)
            self.u_FREE = torch.tensor(FREE[:, 4:5], dtype=torch.float32, device=self.device)
            self.v_FREE = torch.tensor(FREE[:, 5:6], dtype=torch.float32, device=self.device)
            self.w_FREE = torch.tensor(FREE[:, 6:7], dtype=torch.float32, device=self.device)
        else:
            self.x_FREE = self.y_FREE = self.z_FREE = self.t_FREE = None
            self.u_FREE = self.v_FREE = self.w_FREE = None

        self.uv_layers = list(uv_layers)
        if int(self.uv_layers[0]) != int(self.model_input_dim):
            print(f"Adjusting NN input dimension from {self.uv_layers[0]} to {self.model_input_dim}.")
            self.uv_layers[0] = self.model_input_dim
    ...
    def _record_losses(self, loss, loss_f, loss_blade, loss_inlet, loss_far, loss_outlet, loss_ic, loss_free) -> None:
        ...
        self.loss_components_rec["free"].append(self._loss_scalar(loss_free))
    ...
    def total_loss(self):
        ...
        if self.has_free_anchor:
            u_free, v_free, w_free, _, _, _, _, _, _, _ = self.net_uvw(
                self.x_FREE,
                self.y_FREE,
                self.z_FREE,
                self.t_FREE,
            ...
            loss_free = (
                torch.mean(torch.square(u_free - self.u_FREE))
                + torch.mean(torch.square(v_free - self.v_FREE))
                + torch.mean(torch.square(w_free - self.w_FREE))
            )
        else:
            loss_free = torch.zeros((), dtype=torch.float32, device=self.device)
        loss = (
            loss_f
            ...
            + self.loss_weight_free * loss_free
        )
        return loss, loss_f, loss_blade, loss_inlet, loss_far, loss_outlet, loss_ic, loss_free

    def train(self, iters, learning_rate):
        self.optimizer_Adam.param_groups[0]["lr"] = learning_rate
        for it in range(iters):
            self.optimizer_Adam.zero_grad()
            loss, loss_f, loss_blade, loss_inlet, loss_far, loss_outlet, loss_ic, loss_free = self.total_loss()
            loss.backward()
            self.optimizer_Adam.step()
            self._record_losses(loss, loss_f, loss_blade, loss_inlet, loss_far, loss_outlet, loss_ic, loss_free)
            if it % 10 == 0:
                print(
                    ...
                    f"ic={loss_ic.item():.3e}, free={loss_free.item():.3e}"
                )

    def callback(self, loss, loss_f, loss_blade, loss_inlet, loss_far, loss_outlet, loss_ic, loss_free):
        self.count += 1
        self._record_losses(loss, loss_f, loss_blade, loss_inlet, loss_far, loss_outlet, loss_ic, loss_free)
        print(
            f"{self.count} th LBFGS iterations, Loss: {self._loss_scalar(loss):.6e}, "
            f"free={self._loss_scalar(loss_free):.6e}"
        )
    ...
    def train_lbfgs(self, max_iter=50000, history_size=20):
        ...
        def closure():
            ...
            loss, loss_f, loss_blade, loss_inlet, loss_far, loss_outlet, loss_ic, loss_free = self.total_loss()
            loss.backward()
            self.callback(loss, loss_f, loss_blade, loss_inlet, loss_far, loss_outlet, loss_ic, loss_free)
            return loss
...
if __name__ == "__main__":
    ...
    # 2) the wall target velocity is omega(t) x r
    # 3) interior collocation points inside the ramped rotating blade are removed in time
    ...
    T_MIN = 0.0
    # Use an explicit 5 s window for export and diagnostics.
    ...
    # Current setup: 0-5 s ramps from 0 to U_MAX and target omega, then 5-20 s stays at targets.
    ...
    LOSS_WEIGHT_OUTLET = 1.0
    LOSS_WEIGHT_IC = 2.0
    USE_FREESTREAM_ANCHOR = True
    N_FREE_ANCHOR = 4000
    LOSS_WEIGHT_FREE = 2.0
    FREE_ANCHOR_RADIAL_MIN_R = 2.2
    USE_ROTOR_AWARE_DECOMPOSITION = True
    ...
    UV_INPUT_DIM = 4
    UV_LAYERS = [UV_INPUT_DIM] + 8 * [88] + [10]
    ADAM_ITERS = 12000
    LBFGS_MAX_ITER = 2000
    ...
    SLICE_OUTPUT_DIR = f"{OUTPUT_DIR}/orthogonal_slices"


    ensure_dir(OUTPUT_DIR)
    ensure_dir(SLICE_OUTPUT_DIR)
    ...
    rotor_motion_kwargs = {
        "t_ref": T_MIN,
        "motion_mode": INFLOW_MODE,
        "ramp_fraction": RAMP_FRACTION,
        "ramp_tmax": T_MAX,
    }
    rotor_angle_total = float(
        rotor_phase_at_time(
            T_MAX,
            T_MIN,
            T_MAX,
    ...
    rotor_revolutions_total = rotor_angle_total / (2.0 * np.pi)
    ramp_duration_actual = max(float(RAMP_FRACTION), 1e-6) * max(T_MAX - T_MIN, 1e-12)
    ...
    print(
        ...
        f"TSR={float(TSR_TARGET):.6g}, target RPM={rotor_rpm:.6g}, target omega={omega_rad_s:.6g} rad/s, "
        f"tip_speed={tip_speed:.6g} m/s, period={rotor_period:.6g} s"
    )
    print(
        f"Rotor omega profile: synced with INFLOW_MODE={INFLOW_MODE}, ramp_duration={ramp_duration_actual:.6g} s, "
        f"ramped_revolutions={rotor_revolutions_total:.6g}"
    )
    ...
    print(f"UV input dimension={UV_INPUT_DIM}")
    static_mesh = None
    ...
    OUTLET = remove_points_inside_obstacles(
        OUTLET,
        blade_mesh,
        rotor_center,
        rotor_axis,
    ...
    IC = remove_points_inside_obstacles(
        IC,
        blade_mesh,
        rotor_center,
        rotor_axis,
    ...
    FREE = np.empty((0, 7), dtype=float)
    if USE_FREESTREAM_ANCHOR:
        FREE = build_freestream_anchor_dataset(
            domain=domain,
            rotor_center=rotor_center,
            rotor_axis=rotor_axis,
            rotor_radius=rotor_radius,
            rotating_mesh=blade_mesh,
            omega_rad_s=omega_rad_s,
            tmin=T_MIN,
            tmax=T_MAX,
            n_samples=N_FREE_ANCHOR,
            u_max=U_MAX,
            inflow_mode=INFLOW_MODE,
            ramp_fraction=RAMP_FRACTION,
            static_mesh=static_mesh,
            radial_min_r=FREE_ANCHOR_RADIAL_MIN_R,
            rotor_motion_kwargs=rotor_motion_kwargs,
        )
    ...
    XYZT_c = remove_points_inside_obstacles(
        XYZT_c,
        blade_mesh,
        rotor_center,
        rotor_axis,
    ...
    print(
        f"Time window (s): [{T_MIN:.6g}, {T_MAX:.6g}] "
        f"({(T_MAX - T_MIN) / rotor_period:.4g} target-speed periods, {rotor_revolutions_total:.4g} ramped revolutions)"
    )
    print(f"Fluid properties: rho={RHO:.6g} kg/m^3, mu={MU:.6g} Pa*s")
    print(
        "Loss weights: blade={:.3g}, inlet={:.3g}, far={:.3g}, outlet={:.3g}, ic={:.3g}, free={:.3g}".format(
            LOSS_WEIGHT_BLADE,
            LOSS_WEIGHT_INLET,
            LOSS_WEIGHT_FAR,
            LOSS_WEIGHT_OUTLET,
    ...
    print(
        f"Freestream anchor: {'enabled' if USE_FREESTREAM_ANCHOR else 'disabled'}, "
        f"points={FREE.shape[0]}, radial_min={FREE_ANCHOR_RADIAL_MIN_R:.3g}R"
    )
    print(
    ...
    model = PINNBladeRotatingUnsteady3D(
        ...
        loss_weight_free=LOSS_WEIGHT_FREE,
    ...
    probe_phase = rotor_phase_at_time(
        t_hist[:, 0],
        T_MIN,
        T_MAX,
        omega_rad_s,
    ...
    probe_xyz = rotate_points_about_axis_time(
        np.tile(base_probe.reshape(1, 3), (t_hist.shape[0], 1)),
        rotor_center,
        rotor_axis,
        probe_phase,
    )
    ...
    for stale_slice in Path(SLICE_OUTPUT_DIR).glob("rotating_blade3d_orthogonal_slices_t_*.png"):
        stale_slice.unlink()
    slice_files = []
    ...
    for ti in slice_times:
        ...
        outfile = f"{SLICE_OUTPUT_DIR}/rotating_blade3d_orthogonal_slices_t_{ti:.4f}.png"
        plot_orthogonal_slices_unsteady(
            model,
            plot_mesh_t,
            plot_domain,
    ...
    print(
        "Final losses -> total: {:.3e}, phys: {:.3e}, blade: {:.3e}, inlet: {:.3e}, far: {:.3e}, outlet: {:.3e}, ic: {:.3e}, free: {:.3e}".format(
            final_loss[0].item(),
            final_loss[1].item(),
            final_loss[2].item(),
            final_loss[3].item(),
